...
import { FiSearch, FiUser, FiSend, FiPaperclip, FiMoreVertical, FiMessageSquare } from "react-icons/fi";
...
export default function ChatPage() {
  ...
  // Load User List on Mount + Intercept URL Parameter
  useEffect(() => {
    const loadUsersAndHandleUrl = async () => {
      setLoading(true);
      try {
        ...
        // If ?contact=X parameter is present, look for that contact or inject them
        if (contactIdFromUrl) {
          ...
          if (foundUser) {
            ...
            // Fallback placeholder fetch if user isn't in your recent chats list yet
            try {
              ...
              if (res.ok && freshUser) {
                // Prepend new contact onto the side panel array list dynamically
                setUsers((prev) => [freshUser, ...prev]);
                setActiveContact(freshUser);
              }
            } catch (err) {
              console.error("Could not trace dynamic contact fallback initialization profiles:", err);
            }
          }
        }
      } catch (err) { 
    ...
    loadUsersAndHandleUrl();
  }, [contactIdFromUrl]);

  // Fetch real conversation from Laravel when a contact is selected
  useEffect(() => {
    ...
    const loadConversation = async () => {
      try {
        ...
        console.error("Could not load database messages:", err);
      }
    };

    loadConversation();

    // Setup micro-polling interval to keep conversation streams updated smoothly
    const syncInterval = setInterval(loadConversation, 4000);
    return () => clearInterval(syncInterval);
  }, [activeContact]);

  const filteredUsers = useMemo(() => {
    return users.filter(u => `${u.first_name || ""} ${u.last_name || ""}`.toLowerCase().includes(searchTerm.toLowerCase()));
  }, [searchTerm, users]);

  // Sends payload to Laravel API
  const handleSendMessage = async () => {
    ...
    try {
      ...
      console.error("Message failed to sync with database:", err);
      setNewMessage(textToSend); 
      alert("Something went wrong. Message couldn't be saved.");
    }
  };
      ...
      <div className="flex flex-1 pt-16 overflow-hidden">
        ...
        <aside className="w-80 bg-[#051F20] border-r border-[#163832] flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              <div className="p-4 text-[#5a7771] text-sm text-center">Loading chats...</div>
            ) : filteredUsers.map((user) => (
              <div 
                key={user.id} 
                onClick={() => setActiveContact(user)}
                className={`p-4 flex items-center gap-4 cursor-pointer transition-all border-l-4 ${activeContact?.id === user.id ? 'bg-[#0B2B26] border-[#235347]' : 'border-transparent hover:bg-[#082421]'}`}
              >
                {/* User avatar item in sidebar */}
                {user.avatar ? (
                  <img src={`http://127.0.0.1:8000/storage/${user.avatar}`} alt="profile" className="w-12 h-12 rounded-full object-cover border border-[#235347]" />
                ) : (
                ...
                <div className="min-w-0 flex-1">
                  <h4 className="font-semibold text-[#DAF1DE] truncate">
                    {user.first_name || user.name ? `${user.first_name || user.name} ${user.last_name || ""}` : "User Details"}
                  </h4>
                </div>
              </div>
            ))}
          ...
          {activeContact ? (
            <>
              ...
              <header className="px-6 py-4 bg-[#051F20] border-b border-[#163832] flex justify-between items-center">
                <h3 className="font-bold text-[#DAF1DE] flex items-center gap-3">
                  {activeContact.avatar ? (
                    <img src={`http://127.0.0.1:8000/storage/${activeContact.avatar}`} alt="active-profile" className="w-8 h-8 rounded-full object-cover" />
                  ) : (
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-6">
                {messages.map((m) => (
                  <div key={m.id} className={`flex gap-3 items-end ${m.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
                    
                    {/* Render Other User Avatar left of message bubble */}
                    {m.sender !== 'me' && (
                      activeContact.avatar ? (
                        <img src={`http://127.0.0.1:8000/storage/${activeContact.avatar}`} alt="" className="w-8 h-8 rounded-full object-cover mb-1" />
                      ) : (
                        <div className="w-8 h-8 rounded-full bg-[#163832] flex items-center justify-center text-[#8EB69B] text-xs font-bold mb-1 shrink-0">
                          {(activeContact.first_name || activeContact.name || "U").charAt(0).toUpperCase()}
                        </div>
                      )
                    )}
                    ...
                    <div className="flex flex-col max-w-[60%]">
                      <div className={`px-5 py-3 rounded-2xl shadow-sm ${m.sender === 'me' ? 'bg-[#235347] text-white rounded-br-none' : 'bg-[#051F20] border border-[#163832] text-[#DAF1DE] rounded-bl-none'}`}>
              ...
              <footer className="p-4 bg-[#051F20] border-t border-[#163832]">
                <div className="flex items-center gap-3 bg-[#0B2B26] rounded-2xl p-2 border border-[#163832]">
                  <button type="button" className="p-3 text-[#5a7771] hover:text-white"><FiPaperclip /></button>
                  <input 
                    value={newMessage} 
                  ...
                  />
                  <button type="button" onClick={handleSendMessage} className="p-3 bg-[#235347] text-white rounded-xl hover:bg-[#3a685d] flex items-center justify-center shrink-0"><FiSend /></button>
                </div>
              </footer>
            </>
          ) : (
